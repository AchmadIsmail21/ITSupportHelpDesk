...
namespace Client.Repository.Data
{
    public class CaseRepository : GeneralRepository<Case, int>
    {
        ...
        public async Task<List<CaseVM>> GetHistoryTicketsByUserId(int userId)
        {
            ...
            using (var response = await httpClient.GetAsync(request + "ViewHistoryByUserId/" + userId))
            {
                string apiResponse = await response.Content.ReadAsStringAsync();
                data = JsonConvert.DeserializeObject<List<CaseVM>>(apiResponse);
            }
        ...
        public async Task<List<CaseVM>> GetHistoryTicketsByStaffId(int staffId)
        {
            ...
            using (var response = await httpClient.GetAsync(request + "ViewHistoryByStaffId/" + staffId))
            {
                string apiResponse = await response.Content.ReadAsStringAsync();
                data = JsonConvert.DeserializeObject<List<CaseVM>>(apiResponse);
            }
        ...
        public async Task<List<CaseVM>> GetTicketsByLevel(int caseId)
        {
            List<CaseVM> data = new List<CaseVM>();

            using (var response = await httpClient.GetAsync(request + "ViewTicketByLevel/" + caseId))
            {
                string apiResponse = await response.Content.ReadAsStringAsync();
                data = JsonConvert.DeserializeObject<List<CaseVM>>(apiResponse);
            }
